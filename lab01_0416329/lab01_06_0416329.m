...
fprintf('\n');
fprintf('Student Name:Alden Rivera ID:0416329\n');

[x,y] = meshgrid([-10:0.5:10]);     %z = 2 e^(-(y-5)^2) sin(x) + e^(-x^2) cos(y)

z = 2.*exp(-1.*(y-5).^2).*sin(x) + exp(-1.*x.^2).*cos(y);
while true
    clf;
    surf(z);
    zlim([-2,2]);
    n = input('Press enter to smooth.');
    sx = size(x);
    sy = size(y);
    ...
    for i=2:(ix-1)
        for j=2:(iy-1)
            ...
            z1(i, j) = sum(z((i-1):(i+1), (j-1):(j+1)),'all')/9;
        end
    end
    z = z1;
end
